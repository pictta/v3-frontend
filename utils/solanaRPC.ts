...
export default class SolanaRpc {
    ...
    signMessage = async (): Promise<string> => {
        try {
            ...
            const res = await solanaWallet.signMessage(new Uint8Array(msg));
            return res.toString();
        } catch (error) {
            return error as string;
        }
    ...
    sendSOL = async (fromAddress: string, toAddress: string, amount: number): Promise<string> => {
        try {
            const solanaWallet = new SolanaWallet(this.provider);
            const accounts = await solanaWallet.requestAccounts();

            // Ensure that the fromAddress is one of the connected accounts
            if (!accounts.includes(fromAddress)) {
                throw new Error('From address must be one of the connected accounts.');
            }
